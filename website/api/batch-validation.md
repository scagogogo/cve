...
`ValidateCves` returns a per-item verdict, short-circuiting at the first failure:

```mermaid
flowchart TD
    C["each CVE"] --> IC{"IsCve()?"}
    IC -- No --> E1["invalid CVE format"]
    IC -- Yes --> YR{"year 1999..current?"}
    YR -- No --> E2["year before 1999 /<br/>after current year"]
    YR -- Yes --> SQ{"seq &gt; 0?"}
    SQ -- No --> E3["sequence number<br/>must be positive"]
    SQ -- Yes --> OK["Valid ✓"]
```
