...
The seven API modules and how a typical CVE workflow connects them:

```mermaid
flowchart LR
    GEN["Generation"] --> FV["Format & Validation"]
    EX["Extraction"] --> FV
    FV --> CS["Comparison & Sorting"]
    FV --> FG["Filtering & Grouping"]
    FG --> SO["Set Operations"]
    FV --> BV["Batch Validation"]
    FG --> ST["Statistics"]
    FG --> RP["Range & Pattern"]
```
...
| `FormatSeq(cve string, width int) string` | Zero-pad the sequence number to a fixed width |
...
| `IsCveYearOk(cve string) bool` | Check CVE year is within 1999–current year |
| `IsCveYearOkWithCutoff(cve string, cutoff int) bool` | Year check tolerating N future years |
...
| `GenerateFakeCve() string` | Generate a random current-year CVE (for tests) |
...
- **Package Metadata**: The `Version` variable — link-time injection via ldflags — [View Docs](/api/functions/version)
