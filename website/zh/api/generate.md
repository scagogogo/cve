...
两个入参直接进入 `Format`，中间没有任何验证：

```mermaid
flowchart LR
    Y["year (int)"] --> F["fmt.Sprintf<br/>CVE-%d-%d"]
    S["seq (int)"] --> F
    F --> FM["Format()<br/>大写 + 去空格"]
    FM --> O["CVE-YYYY-NNNN"]
```
...
生成一个「当前年份 + 随机序列号」的 CVE，主要用于测试和演示。
...
- `string`: 当前年份、序列号在 `10000`–`99999` 之间随机的 CVE 编号

### 功能描述

`GenerateFakeCve` 以 `time.Now().Year()` 作为年份，用当前纳秒值推导序列号，因此连续调用通常会得到不同结果。它是 `GenerateCve` 的便捷封装，仅用于生成一次性测试数据 —— 切勿用于真实 CVE 分配。

```mermaid
flowchart LR
    N["time.Now()"] --> Y["Year()<br/>当前年份"]
    N --> NS["Nanosecond() % 90000"]
    NS --> SEQ["+ 10000<br/>范围 10000–99999"]
    Y --> G["GenerateCve(year, seq)"]
    SEQ --> G
    G --> O["CVE-YYYY-NNNNN<br/>(一次性)"]
```
...
fmt.Println(fake) // 例如 CVE-2023-42137
...
::: tip 提示
需要把序列号补零对齐时，配合 [`FormatSeq`](/zh/api/format-validate#formatseq) 使用：
...
由于 `GenerateCve` 不做任何验证，推荐的模式是先生成再验证，丢弃无效结果：
