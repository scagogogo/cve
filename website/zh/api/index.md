...
| `FormatSeq(cve string, width int) string` | 将序列号补零到固定宽度 |
...
| `IsCveYearOk(cve string) bool` | 判断 CVE 年份是否在 1999~当前年份 |
| `IsCveYearOkWithCutoff(cve string, cutoff int) bool` | 带未来偏移量的年份校验 |
...
| `GenerateFakeCve() string` | 生成当前年份的随机 CVE（测试用） |
...
- **包元数据**: `Version` 变量 —— 通过 ldflags 链接期注入 — [查看文档](/zh/api/functions/version)
...
- [格式化与验证](/zh/api/format-validate) - 学习如何验证和标准化 CVE
- [提取方法](/zh/api/extract) - 了解如何从文本中提取 CVE 信息
- [比较与排序](/zh/api/compare-sort) - 掌握 CVE 比较和排序技巧
- [过滤与分组](/zh/api/filter-group) - 学习高级的过滤和分组操作
- [生成与构造](/zh/api/generate) - 了解如何生成新的 CVE 编号

或者查看 [使用示例](/zh/examples/) 了解实际应用场景。
