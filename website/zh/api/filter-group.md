...
同一份 CVE 列表可派生出多种专用视图：

```mermaid
flowchart TD
    L["CVE 列表"] --> FY["FilterCvesByYear()<br/>单一年份"]
    L --> FR["FilterCvesByYearRange()<br/>年份区间"]
    L --> GR["GetRecentCves()<br/>最近 N 年"]
    L --> GB["GroupByYear()<br/>map: 年份 → CVE"]
    L --> RD["RemoveDuplicateCves()<br/>去重"]
```
