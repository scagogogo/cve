...
```mermaid
flowchart LR
    T["自由文本"] --> EX["提取方法<br/>ExtractCve / Split"]
    EX --> FV["格式化与验证<br/>Format / IsCve / ValidateCve"]
    FV --> CS["比较与排序<br/>SortCves / CompareCves"]
    FV --> FG["过滤与分组<br/>GroupByYear / FilterByYear"]
    GEN["生成与构造<br/>GenerateCve"] -.-> FV
```
...
下图追踪一段原始安全公告文本，依次经过格式化、验证、分解，最终收敛为排序后的分组报告。每个方框标注了具体涉及的函数，侧边分支展示了验证与分解如何接入主流程。

```text
+---------------+     +------------------+     +-------------------+     +------------------+
| 原始公告      | --> | ExtractCve       | --> | Format (ToUpper + | --> | RemoveDuplicate  |
| "见 CVE-2022 |     | cveRegex 捕获组  |     |  TrimSpace) 逐条  |     | Cves (map 去重,  |
|  -12345 ..."  |     | (?i)(CVE-\d+-\d+) |    |  处理             |     |  保留首次出现)   |
+---------------+     +------------------+     +-------------------+     +------------------+
                                                                                        |
                                                                                        v
+---------------------+     +-------------------+     +------------------+     +------------+
| GroupByYear         | <-- | SortCves          | <-- | ValidateCve 门禁 | <-- | ExtractCve |
| map[year][]string   |     | sort.Slice +      |     | 年份在 [1999,    |     | 年份/序列号 |
| (O(n)，顺序为       |     | CompareCves       |     |  now]；seq > 0   |     | Split()    |
|  插入顺序)          |     | (O(n log n))      |     +-------------------+     +------------+
+---------------------+     +-------------------+
```

第二张图从另一个视角展示比较与集合运算函数如何复用同一组 `ExtractCveYearAsInt` / `ExtractCveSeqAsInt` 原语，以及 `GetRecentCves` 如何委托给年份范围过滤。

```mermaid
flowchart TD
    FMT["Format<br/>ToUpper + TrimSpace"] --> SPLIT["Split<br/>年份、序列号字符串"]
    SPLIT --> YI["ExtractCveYearAsInt"]
    SPLIT --> SI["ExtractCveSeqAsInt"]
    YI --> CBY["CompareByYear<br/>返回年份差值"]
    YI --> SUB["SubByYear<br/>CompareByYear 的别名"]
    CBY --> CC["CompareCves<br/>归一化为 -1/0/1"]
    SI --> CC
    CC --> SORT["SortCves<br/>sort.Slice, O(n log n)"]
    CBY --> FBRR["FilterCvesByYearRange<br/>闭区间边界"]
...
- **两条正则、两种语义。** `base.go` 中 `exactCveRegex` 带有 `^\s*CVE-\d+-\d+\s*$` 锚点，因此 `IsCve` 只接受「本身就是 CVE」（允许两侧空白）的字符串；`containsCveRegex` 去掉锚点（`(?i)CVE-\d+-\d+`），让 `IsContainsCve` 能扫描正文。`extract.go` 又定义了第三条 `cveRegex` = `(?i)(CVE-\d+-\d+)`，逻辑相同但包了一层捕获组，使 `FindAllString` 只返回 CVE 子串——这也是 `ExtractCve` 随后逐条就地 `Format` 而无需重扫原文的原因。

- **`CompareByYear` 与 `CompareCves` 的返回值差异。** `CompareByYear`（compare.go:41）返回的是「原始年份差值」`ExtractCveYearAsInt(a) - ExtractCveYearAsInt(b)`，所以 `CVE-2020-1111` 对 `CVE-2022-2222` 得到 `-2` 而非 `-1`。`CompareCves`（compare.go:110）只借用该差值判断符号，再压平为 `-1/0/1`，并在年份相同时回落到 `ExtractCveSeqAsInt` 比较序列号。这正是基本用法示例中同一对 CVE 的年份比较输出 `-2`、全面比较却输出 `-1` 的原因。

- **验证结果随时间变化。** `ValidateCve`（base.go:459）拒绝任何落在 `[1999, time.Now().Year()]` 之外的年份，并要求 `seqInt > 0`：`CVE-2022-0` 因序列号规则失败，`CVE-2099-12345` 因上界规则失败——结论取决于调用时的系统时钟。`IsCveYearOkWithCutoff` 用 `cutoff` 偏移放宽上界（`year <= time.Now().Year()+cutoff`），适用于预留或未来的 CVE 编号；`IsCveYearOk` 即该函数 `cutoff == 0` 的特例。

- **去重与交并差共用同一惯用法。** `filter.go` 中四个集合函数（`RemoveDuplicateCves`、`IntersectCves`、`UnionCves`、`DiffCves`）都把每个元素过一遍 `Format`，并以大写后的 CVE 作为 `map[string]struct{}` 的键，因此 `cve-2022-1111` 与 `CVE-2022-1111` 必然合并为一条。`RemoveDuplicateCves` 保留首次出现顺序；三个集合运算则统一返回 `SortCves(result)`，调用方无需再排序即可得到「年份优先、序列号次之」的稳定顺序。

- **`GenerateCve` 刻意跳过校验。** 根据其文档注释（generate.go:42-43），`GenerateCve` 不检查年份是否晚于 1999、序列号是否为正，只执行 `Format(fmt.Sprintf("CVE-%d-%d", year, seq))`。这使其可用于构造历史、测试或刻意越界的编号；需要保证结果「合理」时请配合 `ValidateCve`。`GenerateFakeCve` 正是利用这一点：它用 `10000 + time.Now().Nanosecond()%90000` 作为序列号（generate.go:102），输出随机但从不校验。

## 下一步

现在您已经掌握了 CVE Utils 的基本使用方法，可以：

1. 查看 [API 文档](/zh/api/) 了解所有函数的详细信息
2. 浏览 [使用示例](/zh/examples/) 学习更多实际应用场景
3. 参考具体的 API 分类文档：
   - [格式化与验证](/zh/api/format-validate)
   - [提取方法](/zh/api/extract)
   - [比较与排序](/zh/api/compare-sort)
   - [过滤与分组](/zh/api/filter-group)
   - [生成与构造](/zh/api/generate)
